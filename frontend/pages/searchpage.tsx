...
function SearchBar({ value, onChange }: { value: string; onChange: (v: string) => void }) {
  return (
    <div className="relative w-full max-w-xl">
      <span className="pointer-events-none absolute left-4 top-1/2 h-5 w-5 -translate-y-1/2 text-neutral-400" aria-hidden>🔍</span>
      <input
        type="search"
        placeholder="Search..."
        aria-label="Search"
      ...
      />
      <span className="absolute right-4 top-1/2 h-5 w-5 -translate-y-1/2 text-neutral-400 flex items-center justify-center" aria-hidden>☰</span> 
    </div>
  );
}
...
export default function SearchPage() {
  const { query, sort, tools, loading, error, setQuery, setSort, retry } = useSearch();
